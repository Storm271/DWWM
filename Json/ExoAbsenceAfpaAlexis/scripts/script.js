function debut() {
  var chopperRadio = $("#check-1");
  var chopperRadioBis = $("#check-2");
  var unJour = $("#un-jour");
  var plusJour = $("#plus-jour");
  ...
  genMotif = function (data) {
    ...
    for (x = 0; x < lesCodes.length; x++) {
      //premiere boucle pour mettre chaque code d'absence contenant les motifs
      ...
      for (y = 0; y < lesCodes[x].options.length; y++) {
        if (lesCodes[x].options[y].op_label != "") {
          //deuxieme boucle pour mettre les motifs dans des checkbox pour les ramener a l'autre page
          autrecode.innerHTML +=
            '<li><input type="radio" name="motif" id="' +
            lesCodes[x].options[y].op_id +
            '" value="' +
            lesCodes[x].options[y].op_id +
            '"> <label for="' +
            lesCodes[x].options[y].op_id +
            '">' +
            lesCodes[x].options[y].op_label +
            "</li>";
        }
      }
